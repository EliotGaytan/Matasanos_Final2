...
{   
    ...
    $Porcentaje = number_format($Porciento, 2);
    if($Porciento<1)
    {
    ...
    if ($Porciento<59){
        $Diag_F="<br> Tiene ".$Porcentaje."% de probabilidad de estar enfermo por dengue, por lo que tendria un riesgo considerable de tener dengue,
         si mas sintomas se presentan acuda a su medico preferido";   

    }

    if ($Porciento>59){
        $Diag_F="Tiene ".$Porcentaje."% de probabilidad de estar enfermo por dengue, por lo que tendria un riesgo alto de tener dengue,
    ...
    $result = $dbcon->query("SELECT No_Cons FROM consulta");
    ...
    $result = $dbcon->query("SELECT ID FROM expediente");
    $cont = $result->num_rows;
    $cont+=1;
    
    $sql = "INSERT INTO expediente(ID,FK_Usr,FK_Cons) VALUES
    ('".$cont."','".$id."','".$cont."')";
...
<body>
    <form class="formulario" method="post" enctype="multipart/form-data" >
        ...
        <h1>Contesta el siguiente formulario.</h1>
                    ...
                    <button class="button" formaction="After_login.php">Inicio</button>
                </div>
